...
1. **볼드 번짐 차단**(`insert/dom.ts` `flattenBold`) — 실측서 소제목·키워드 아닌 문장에도 볼드가 연속으로 번지는 문제. 원인 = 마크다운 `**…**` → `<strong>` 가 본문 paste 에 그대로 들어가고, **SE 가 문단 첫머리 볼드를 런 전체·다음 문단까지 번지게** 함(태그 미닫힘 아님 — SE 고유 동작, dom.ts 주석에 실측 기록돼 있던 것). `demoteHeading` 은 h1~h4 만 막아 일반 문단/리스트 시작 볼드는 통과했음. → `splitForSe` 입구에서 본문의 **모든 `<strong>/<b>` 를 평문화**(li·td 안까지 일괄, `querySelectorAll('strong, b')` → `replaceWith(childNodes)`). heading 이 이미 볼드 포기한 것과 같은 결정(SE 는 paste 로 깔끔한 볼드 못 받음). ⚠️ **트레이드오프**: 의도한 키워드 볼드도 사라짐 — 진짜 볼드는 후속 SE 툴바 조작 경로 필요(별건). 테스트 2케이스 추가(`insert-engine.test.ts`: 문단 시작 볼드·li 안 볼드 평문화).
2. **등록 어투 편집**(`App.tsx`) — 어투 드롭다운서 저장된 프로필 고르면 `onPickVoice` 가 명세·발췌·이름·블로그ID 를 편집칸에 로드 → 수정 후 `저장`(같은 이름 덮어쓰기). `어투 없음` 선택·삭제 시 편집칸 비움. (기존엔 학습 직후에만 편집 가능했음.)
3. 검증: tsc 0, 테스트 110 pass(108+2), prod 빌드 OK. **브라우저 실측 미확인**(볼드 번짐 해소·어투 편집 라이브).
