...
1. **WP4 나머지** — 이미지 소스 모드(업로드/AI/외부), H1 대표 썸네일·본문(IMG) 슬롯. AI/외부는 키·어댑터 필요.
2. **WP6 모델 일관성** — 참조 이미지 등록 + AI 생성 시 동반 전송(R-7.7).
3. **WP8 8-2** — 썸네일에 링크 부착(현재 H2THUMB 무링크라 보류).

> 키 불필요한 핵심(WP1·2·4·5·7·8)은 끝. 남은 건 대부분 AI/외부 **키·어댑터 필요**(WP4 나머지·WP6) → M4 자동화로 넘어갈지 검토.
