...
| WP7 | 정합성 확장(`validate.ts` PENDING_M3_CHECKS) | ⬜ (이미지 opt-in 으로 축소됨, 아래 참조) |
...
1. **WP7 정합성(축소판)** — 이미지가 opt-in 되어 "마커=Visual 강제차단"은 불필요. 남은 핵심은 **H2 수 === H2THUMB 수**(R-7.3) 안전망. `validate.ts` 에 `contentHtml`(또는 H2 개수) 넘겨 검사 추가. 규모 작음.
2. **WP5 중복 회피 + 압축** — 같은 썸네일이라도 매번 다른 바이트(메타 제거 + 1px 노이즈), 품질 슬라이더(`toBlob(quality)`), 용량 미터(`storage.estimate()`). 네이버 중복 이미지 회피(R-7.4).
3. **WP4 나머지** — 이미지 소스 모드(업로드/AI/외부), H1 대표 썸네일·본문(IMG) 슬롯. AI/외부는 키·어댑터 필요.
4. **WP6 모델 일관성** — 참조 이미지 등록 + AI 생성 시 동반 전송(R-7.7).
5. **WP8 8-2** — 썸네일에 링크 부착(현재 H2THUMB 무링크라 보류).

> 추천 착수: **WP7(작고 안전) → WP5(체감 큼, 키 불필요)**.
...
- [ ] 이미지 마커↔Visual 정합성 / H2↔H2THUMB (WP7)
