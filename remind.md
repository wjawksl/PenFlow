...
| WP4 | ⑨ 비주얼 생성 — H2 썸네일(기본 Canvas + AI Gemini) + 업로드 | 🟡 A3 진행(소스: 기본·AI·업로드 ✅ / H1 대표·본문 IMG 슬롯 ⬜) |
...
1. **WP4 A3 잔여** — 소스 모드(업로드·AI ✅, 외부 EXTERNAL 제외). 남은 건 **H1 대표 썸네일·본문(IMG) 슬롯**. 본문 IMG 는 생성기가 IMG 마커 emit 해야 함.
2. **WP6 모델 일관성** — 참조 이미지 등록 + AI 생성 시 동반 전송(R-7.7). 어댑터에 `modelRef` inlineData 자리 이미 마련.
3. **WP8 8-2** — 썸네일에 링크 부착(현재 H2THUMB 무링크라 보류).
4. **브라우저 실측** — AI 썸네일 생성(Gemini 키), 업로드 풀, 모델명(`gemini-2.5-flash-image`) 유효성. 코드만 통과, 실호출 미검증.

> A3 로 소스 모드(기본·AI·업로드)는 종단 배선됨. 남은 M3 는 슬롯 확장(WP4)·WP6·WP8 8-2. 또는 M4(자동화+발행)로 점프 가능.
