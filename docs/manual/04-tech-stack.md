...
> **⚠️ M3 실측 결과 — Kiwi 보류 (2026-06-06)**: `kiwi-nlp` 0.23.0 wasm(3.6MB)은 토큰화 정상 동작하나, **모델이 필수이고 가장 작은 base 모델도 `cong.mdl` 75MB 포함 → 최소 ~94MB(압축해제)**. npm 패키지엔 모델 미포함(Kiwi 릴리스에서 별도 조달). MV3 확장 번들엔 비현실적, 원격+캐시도 첫 다운로드 ~80MB. 밀도검사 1개 기능 대비 과해 **WP2 는 경량 정규화 카운트로 구현**(타깃 키워드는 이미 아는 명사라 형태소 없이 동일 결과). 상세는 `milestone/M3-작업분해.md` WP2 구현 현황. 정확도(명사 자동발굴·용언 활용 통합)가 필요해지면 Kiwi 를 Offscreen 로드로 재도입 가능하게 함수 경계를 분리해 둠.
